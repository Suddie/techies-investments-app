...
import { format, parse, addMonths, startOfMonth, setDate } from 'date-fns';
import { Skeleton } from '@/components/ui/skeleton';
import { AlertTriangle, CheckCircle, ClipboardList } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
// Import for future actions:
// import { Button } from "@/components/ui/button";
// import { MoreHorizontal, Edit2, Trash2, XCircle } from "lucide-react";
...
export default function AllContributionsTable() {
  ...
  useEffect(() => {
    ...
    // Order by datePaid descending to show recent ones first
  ...
  // Client-side estimation for visual cue of lateness (can be refined)
  const isVisuallyLate = (contrib: Contribution): boolean => {
    ...
    try {
      ...
      const dueDate = setDate(monthFollowingFirstMonth, 7); // 7th of next month
  ...
  if (loading) {
    return (
      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle>Loading All Contributions...</CardTitle>
          <CardDescription>Fetching payment history for all members.</CardDescription>
        </CardHeader>
     ...
     return (
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>No contributions found from any member yet.</CardDescription>
        </CardHeader>
        <CardContent>
          <p className="text-center text-muted-foreground py-8">
            <ClipboardList className="mx-auto h-12 w-12 text-muted-foreground/50 mb-2" />
            The contributions log is currently empty.
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card className="w-full shadow-lg">
      <CardHeader>
        ...
        <CardDescription>A comprehensive record of all contributions received.</CardDescription>
      </CardHeader>
      <CardContent>
        <Table>
          <TableHeader>
            <TableRow>
              ...
              {/* Add Actions column header later when implementing actions */}
                  ...
                  {/* <TableCell>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem className="text-destructive">Void/Delete</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                  </TableCell> */}
                </TableRow>
      ...
      {contributions.length > 15 && ( // Show footer if many entries
        <CardFooter>
